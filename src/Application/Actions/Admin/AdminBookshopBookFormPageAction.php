...
class AdminBookshopBookFormPageAction extends AbstractAdminBookshopAction
{
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        $resolvedSku = $isEdit
            ? (string) ($existingBook['sku'] ?? '')
            : $this->bookshopRepository->generateNextBookSku();
        $payload = $this->normalizePayload($body, $resolvedSku);
     ...
     */
    private function normalizePayload(array $input, string $forcedSku = ''): array
    {
        $title = BookshopTextNormalizer::normalizeTitle((string) ($input['title'] ?? ''));
        $sku = $forcedSku !== ''
            ? $this->normalizeBookshopSku($forcedSku)
            : $this->normalizeBookshopSku($input['sku'] ?? '');
    ...
    private function renderForm(
        ...
        $form = [
            'sku' => $submittedPayload['sku'] ?? ($existingBook['sku'] ?? $generatedSku),
        ...
        return $this->renderPage($response, 'pages/admin-bookshop-book-form.twig', [
            ...
            'bookshop_book_form_sku_autogenerated' => true,
