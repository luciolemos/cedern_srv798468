...
class AdminAccessDataPageAction extends AbstractPageAction
{
    ...
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        if (empty($errors)) {
            try {
                ...
                $errors[] = 'Não foi possível salvar Dados de acesso. Tente novamente.';
            } catch (\Throwable $exception) {
                $this->logger->warning('Falha ao salvar dados de acesso no painel.', [
                    'error' => $exception->getMessage(),
                ]);

                $errors[] = 'Erro ao salvar Dados de acesso. Verifique a conexão e tente novamente.';
            }
        }
    ...
    private function validatePayload(array $payload): array
    {
        ...
        if ($payload['title'] === '') {
            $errors[] = 'Informe o título da página de Dados de acesso.';
        }

        if ($payload['body'] === '') {
            $errors[] = 'Informe o conteúdo de Dados de acesso.';
        }
    ...
    private function loadDocumentRecord(): array
    {
        ...
        try {
            ...
            $this->logger->warning('Falha ao carregar dados de acesso no painel.', [
                'error' => $exception->getMessage(),
            ]);
        }
