...
class LibraryManagedFilePageAction extends AbstractAdminLibraryAction
{
    public function __invoke(Request $request, Response $response): Response
    {
        ...
        if (!in_array($bucket, ['docs', 'capas'], true)) {
        ...
        $relativePath = 'media/biblioteca/' . $bucket . '/' . $fileName;
        $absolutePath = $bucket === 'docs'
            ? $this->resolveManagedLibraryPdfAbsolutePath($relativePath)
            : $this->resolveManagedLibraryCoverAbsolutePath($relativePath);
        ...
        if ($contents === false) {
            $this->logger->warning('Falha ao ler arquivo gerenciado da biblioteca.', [
                'path' => $absolutePath,
            ]);
